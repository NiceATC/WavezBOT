...
async function _initUsersTables() {
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS user_marriage (
    user_id               TEXT PRIMARY KEY,
    spouse_user_id        TEXT,
    married_at            INTEGER NOT NULL DEFAULT 0,
    divorce_penalty_until INTEGER NOT NULL DEFAULT 0,
    updated_at            INTEGER NOT NULL DEFAULT 0
  )`);
  db.exec(
    "CREATE INDEX IF NOT EXISTS idx_user_marriage_spouse ON user_marriage(spouse_user_id)",
  );
}
