...
export class WavezBot {
  ...
  _startPipeline() {
    ...
    this._pipeline.on(Events.WS_CLOSE, (packet) => {
      ...
      const delay = Math.min(5_000 * this._reconnectAttempts, 60_000);
      this._log(
        "info",
      ...
      this._reconnectTimer = setTimeout(() => {
        this._reconnectTimer = null;
        this._login()
          .then(() => this._startPipeline())
          .catch((err) => {
            this._log(
              "warn",
              this.t("logs.bot.reconnectFailed", { error: err.message }),
            );
            // Schedule next retry if not intentionally stopped
            if (!this._intentionalStop) {
              const nextDelay = Math.min(
                5_000 * this._reconnectAttempts,
                60_000,
              );
              this._reconnectTimer = setTimeout(() => {
                this._reconnectTimer = null;
  ...
  async sendReply(content, messageId) {
    if (!this._api?.roomBot?.sendMessage) return null;
    const body = { content };
    if (messageId) body.replyTo = { id: String(messageId) };
  ...
  wsReorderQueue(userId, position) {
    ...
    this._pipeline.send("reorder_queue", {
      roomId: this._roomId,
      targetUserId: String(userId),
      toPosition: Math.trunc(Number(position)),
    });
    return true;
  }
