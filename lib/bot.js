...
import {
  ...
  getVipGreetMessage,
  setVipGreetMessage,
  getMarriageState as getMarriageStateStorage,
} from "./storage.js";
...
function extractMessageIdFromSendResult(res) {
...
export class WavezBot {
  ...
  constructor(cfg = loadConfig()) {
    ...
    this._botRole = null;
    /** Room-bot scope set granted by token/state (API 0.1.9+) */
    this._botPermissions = new Set();
  ...
  async _loginWithPassword() {
    ...
    //    Request broad + granular moderation scopes (API 0.1.9 model).
    ...
    this._botPermissions = new Set(
      Array.isArray(tokenData.permissions) ? tokenData.permissions : [],
    );
  ...
  async _joinRoom() {
    ...
    try {
      ...
      const inferredBotRole =
        bot.roomRole ?? bot.actorRoomRole ?? bot.visualRoomRole ?? null;
      if (inferredBotRole) {
        this._botRole = inferredBotRole;
        this._log("info", this.t("logs.bot.roomRole", { role: this._botRole }));
      }
      this._botPermissions = new Set(
        Array.isArray(bot.permissions) ? bot.permissions : [],
      );
    ...
    if (!this._isReconnecting) {
      try {
        ...
        const delayMs = Number(this.cfg.onlineAnnouncementDeleteMs ?? 30_000);
        if (delayMs > 0) {
          const sentId = extractMessageIdFromSendResult(res);
    ...
    if (content.startsWith(prefix)) {
      ...
      if (name) {
        const blockedCommandIds = new Set(
          (this.cfg.commandBlacklistUserIds ?? [])
            .map((id) =>
              String(id ?? "")
                .trim()
                .toLowerCase(),
        ...
        const senderIdNorm = String(_senderUid ?? "")
          .trim()
          .toLowerCase();
        if (
          blockedCommandIds.size > 0 &&
          !_senderUid?.startsWith("room-bot:") &&
          blockedCommandIds.has(senderIdNorm)
        ) {
          this._log(
            "info",
            `[cmd] blocked user ${_senderUid} -> ${prefix}${name}`,
          );
          return;
        }

        const debugAllowedIds = new Set(
          (this.cfg.debugCommandUserIds ?? [])
            .map((id) =>
              String(id ?? "")
                .trim()
                .toLowerCase(),
        ...
        if (
          ...
          !_senderUid?.startsWith("room-bot:") &&
          !debugAllowedIds.has(senderIdNorm)
        ) {
          return;
        }
  ...
  async _maybeSendIntervalMessage() {
    ...
    if (this.cfg.motdEnabled) {
      ...
      if (msg) {
        const res = await this.sendChat(msg);
        const deleteMs = Number(this.cfg.motdDeleteMs ?? 180_000);
        const sentId = extractMessageIdFromSendResult(res);
        if (sentId && deleteMs > 0)
          this.scheduleMessageDelete(sentId, deleteMs);
      }
      return;
    }
  ...
  async getMarriageState(userId) {
    const uid = String(userId ?? "");
    if (!uid) return null;
    return getMarriageStateStorage(uid);
  }
  ...
  async _awardEconomyPointsUnlocked(
    ...
    if (options?.applyMarriagePenalty !== false) {
      const marriageState = await getMarriageStateStorage(uid);
      const penaltyUntil = Number(marriageState?.divorcePenaltyUntil ?? 0) || 0;
      if (penaltyUntil > Date.now()) {
        const penaltyMultiplier = Math.max(
          0,
          Number(this.cfg.marriageDivorcePenaltyMultiplier ?? 0.5) || 0,
        );
        numericAmount *= penaltyMultiplier;
      }
    }
  ...
  async spendEconomyPoints(userId, amount, identity = null, options = {}) {
    return this._queueProgressOp(async () => {
      ...
      if (options?.allowMarriagePool !== true) return null;

      const marriageState = await getMarriageStateStorage(uid);
      const spouseId = String(marriageState?.spouseUserId ?? "");
      if (!spouseId || spouseId === uid) return null;

      const spouseIdentity = this._getUserIdentity(spouseId);
      const spouseBalance = await this._ensureEconomyBalance(
        spouseId,
        spouseIdentity,
      );
      if (current + spouseBalance < delta) return null;

      const takeFromSelf = current;
      const takeFromSpouse = delta - takeFromSelf;
      const nextSelf = 0;
      const nextSpouse = spouseBalance - takeFromSpouse;

      this._economyBalances.set(uid, nextSelf);
      this._economyBalances.set(spouseId, nextSpouse);
      this._queueEconomyPersist(uid);
      this._queueEconomyPersist(spouseId);

      incrementEconomySpent(uid, delta).catch(() => {});
      incrementEconomySpent(spouseId, takeFromSpouse).catch(() => {});
      return nextSelf;
    });
  }
  ...
  async awardXp(userId, amount, identity = null) {
    return this._queueProgressOp(async () => {
      ...
      const delta = toPointsInt(Number(amount || 0) * multiplier);
      if (!delta) return null;
  ...
  async _applyOnlineAccrualForUser(userId) {
    ...
    let economyPerHour = perHour;
    let xpPerHourEffective = xpPerHour;
    const marriageState = await getMarriageStateStorage(uid);
    const spouseId = String(marriageState?.spouseUserId ?? "");
    const hasPenalty = Number(marriageState?.divorcePenaltyUntil ?? 0) > now;

    if (spouseId && !hasPenalty && this._roomUsers.has(spouseId)) {
      const onlineMultiplier = Math.max(
        1,
        Number(this.cfg.marriageOnlineBonusMultiplier ?? 1.25) || 1,
      );
      economyPerHour *= onlineMultiplier;
      xpPerHourEffective *= onlineMultiplier;
    }

    const identity = this._getUserIdentity(uid);
    if (this.cfg.economyEnabled && economyPerHour > 0) {
      await this.awardEconomyPoints(uid, economyPerHour * hours, identity, {
        applyVipMultiplier: true,
        source: "online",
      });
    }
    if (this.cfg.xpEnabled && xpPerHourEffective > 0) {
      await this.awardXp(uid, xpPerHourEffective * hours, identity);
    }
    return hours;
  }
  ...
  getBotRoleLevel() {
    const roomLevel = getRoleLevel(this._botRole);
    const scopeLevel = getBotPermissionRoleLevel(this._botPermissions);
    return Math.max(roomLevel, scopeLevel);
  }
