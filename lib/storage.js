...
export async function getMarriageState(userId) {
  const uid = String(userId ?? "").trim();
  if (!uid || isRoomBotId(uid)) return null;
  const db = getDb("user_marriage");
  const row = db
    .prepare(
      `SELECT
        user_id AS userId,
        spouse_user_id AS spouseUserId,
        married_at AS marriedAt,
        divorce_penalty_until AS divorcePenaltyUntil,
  ...
  const spouseUserId = row.spouseUserId ? String(row.spouseUserId) : null;
  const marriedAt = Number(row.marriedAt ?? 0) || 0;
  const divorcePenaltyUntil = Number(row.divorcePenaltyUntil ?? 0) || 0;
  return {
    ...
    isMarried: Boolean(spouseUserId && marriedAt > 0),
    hasPenalty: divorcePenaltyUntil > Date.now(),
  };
}

export async function setMarriagePair(
  userAId,
  userBId,
  marriedAt = Date.now(),
) {
  ...
  if (isRoomBotId(a) || isRoomBotId(b)) return false;
  ...
  const db = getDb("user_marriage");
  const now = Date.now();
  const stamp = Math.max(1, Number(marriedAt) || now);

  const upsert = db.prepare(
    `INSERT INTO user_marriage (user_id, spouse_user_id, married_at, divorce_penalty_until, updated_at)
     VALUES (?, ?, ?, 0, ?)
     ON CONFLICT(user_id) DO UPDATE SET
       spouse_user_id = excluded.spouse_user_id,
       married_at = excluded.married_at,
       divorce_penalty_until = 0,
       updated_at = excluded.updated_at`,
  );

  const tx = db.transaction(() => {
    upsert.run(a, b, stamp, now);
    upsert.run(b, a, stamp, now);
  });
  tx();
  return true;
}
...
export async function divorceMarriage({
  ...
  const initiator = String(initiatorUserId ?? "").trim();
  const partner = String(partnerUserId ?? "").trim();
  if (!initiator || !partner || initiator === partner) return false;

  const db = getDb("user_marriage");
  const now = Date.now();
  const until = Math.max(0, Number(penaltyUntil) || 0);

  const clearPartner = db.prepare(
    `INSERT INTO user_marriage (user_id, spouse_user_id, married_at, divorce_penalty_until, updated_at)
     VALUES (?, NULL, 0, 0, ?)
     ON CONFLICT(user_id) DO UPDATE SET
       spouse_user_id = NULL,
  ...
  const clearInitiator = db.prepare(
    `INSERT INTO user_marriage (user_id, spouse_user_id, married_at, divorce_penalty_until, updated_at)
     VALUES (?, NULL, 0, ?, ?)
     ON CONFLICT(user_id) DO UPDATE SET
       spouse_user_id = NULL,
       married_at = 0,
       divorce_penalty_until = MAX(user_marriage.divorce_penalty_until, excluded.divorce_penalty_until),
       updated_at = excluded.updated_at`,
  );

  const tx = db.transaction(() => {
    clearPartner.run(partner, now);
    clearInitiator.run(initiator, until, now);
  });
  tx();
  return true;
}
