...
// Compatibility wrapper: same interface as the sqlite/sqlite3 async API
// but backed by better-sqlite3 (synchronous)
const db = {
  exec(sql) { return _db.exec(sql); },
  run(sql, ...args) { return _db.prepare(sql).run(...args); },
  get(sql, ...args) { return _db.prepare(sql).get(...args); },
  all(sql, ...args) { return _db.prepare(sql).all(...args); },
};
...
export async function executeDbSql(sql, params = []) {
  ...
  const res = db.run(trimmed, params);
  return { changes: res.changes ?? 0, lastId: res.lastInsertRowid ?? null };
}
