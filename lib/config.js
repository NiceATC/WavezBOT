 ...
 *   config/moderation.json  — blacklist, timeguard, AFK, lock, voteskip, DC
...
const CONFIG_FILES = ["core", "bot", "moderation", "economy", "xp", "ui"];
...
if (fs.existsSync(CONFIG_DIR) && fs.statSync(CONFIG_DIR).isDirectory()) {
  ...
  _json = loadJsonFile(configPath, true);
}
